REGISTER './adsafe-pigudfs.jar';
DEFINE REVERSE_URL com.adsafe.pigudfs.urls.REVERSE_URL();
SET default_parallel 250;
S = LOAD 'hbase://q_pages' USING com.adsafe.maged.common.hbase.HBaseStorage('t:d', '-loadKey -startDate=$startdate -endDate=$enddate');
Surl = foreach S generate key.url as url, key.date as date;
Socc = foreach (group Surl by url) {
        dates = Surl.date;
        unique_dates = DISTINCT dates;
        generate CONCAT(group,'') as url2:chararray, COUNT(unique_dates) as occ, COUNT(Surl) as cnt;
};
...
S10mF2 = filter S10mF by NOT ($0 matches '.*/ad/.*' OR $0 matches '.*/ads/.*' OR $0 matches '.*/ads-.*' OR
$0 matches '.*/ad-.*' OR $0 matches '.*/ads..*' OR $0 matches '.*/ad..*' OR $0 matches '.*admanager.*' OR
$0 matches '.*adserving.*' OR $0 matches '.*160x600.*' OR $0 matches '.*728x90.*' OR $0 matches
'.*300x250.*' OR $0 matches '.*468x60.*' OR $0 matches '.*120X240.*' OR $0 matches '.*180x150.*' OR $0
matches '.*120x600.*' OR $0 matches '.*336x280.*' OR $0 matches '.*425x600.*' OR $0 matches '.*234X60.*' 
